...
Ingestion is healthy — the workspace is actively receiving Windows security events. As of this write-up, no failed *RDP* login attempts (the specific event this project is hunting for) have been recorded yet, and so no incident has fired — which is expected: the VM's public IP only became reachable again a short time before this was written, after the auto-shutdown and identity fixes, and internet-wide scanners take anywhere from minutes to a few hours to rediscover an open port. That's recorded here honestly rather than papered over with fabricated test events, in keeping with the whole point of this project: the detections in this repo are proven against real attacker behavior, or they're not proven at all yet, and this document says which is currently true.
